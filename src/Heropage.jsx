...
const Heropage = ({ loggedIn }) => {
  ...
  useEffect(() => {
    const fetchAllPrices = async () => {
      const priceData = await fetchAllPricesFromAPI();

      const prices = { tradable: {}, topGainers: {}, newOnCoinbase: {} };
      const changes = { tradable: {}, topGainers: {}, newOnCoinbase: {} };

      // Process tradable cryptocurrencies
      CRYPTO_CONFIG.tradable.forEach((crypto) => {
        const usdPrice = priceData[crypto.id]?.usd || 0;
        prices.tradable[crypto.id] = convertToGHS(usdPrice);
        changes.tradable[crypto.id] = priceData[crypto.id]?.usd_24h_change || 0;
      });

      // Process top gainers
      CRYPTO_CONFIG.topGainers.forEach((crypto) => {
        const usdPrice = priceData[crypto.id]?.usd || 0;
        prices.topGainers[crypto.id] = convertToGHS(usdPrice);
        changes.topGainers[crypto.id] =
          priceData[crypto.id]?.usd_24h_change || 0;
      });

      // Process new on Coinbase
      CRYPTO_CONFIG.newOnCoinbase.forEach((crypto) => {
        const usdPrice = priceData[crypto.id]?.usd || 0;
        prices.newOnCoinbase[crypto.id] = convertToGHS(usdPrice);
        changes.newOnCoinbase[crypto.id] =
          priceData[crypto.id]?.usd_24h_change || 0;
      });

      setCryptoPrices(prices);
      setCryptoChanges(changes);
      setLoading(false);
    };
    ...
    // Update every second
    const intervalId = setInterval(fetchAllPrices, 1000);
                ...
                {CRYPTO_CONFIG.tradable.map((crypto) => (
                  ...
                  >
                    <div className="flex items-center gap-2.5 h-8">
                      <img src={crypto.img} alt="" className="w-8 h-8" />
                    ...
                    <div className="flex-col h-6 ">
                      ...
                      <span
                        className={` flex justify-end text-[1rem] lg:text-[1.5rem] ${cryptoChanges.tradable[crypto.id] >= 0 ? "text-green-500" : "text-red-500"}`}
                      >
                        {loading
                          ? "..."
                          : formatPercentage(cryptoChanges.tradable[crypto.id])}
                      </span>
                    </div>
                  </div>
                ))}
                ...
                {CRYPTO_CONFIG.topGainers.map((crypto) => (
                  ...
                  >
                    ...
                    <div className="flex-col justify-end h-6">
                      ...
                      <span
                        className={` flex justify-end text-[1rem] lg:text-[1.5rem] ${cryptoChanges.topGainers[crypto.id] >= 0 ? "text-green-500" : "text-red-500"}`}
                      >
                        {loading
                          ? "..."
                          : formatPercentage(
                              cryptoChanges.topGainers[crypto.id],
                            )}
                      </span>
                    </div>
                  </div>
                ...
                {CRYPTO_CONFIG.newOnCoinbase.map((crypto) => (
                  ...
                  >
                    ...
                    <div className="flex-col justify-end h-6">
                      ...
                      <span
                        className={`flex justify-end text-[1rem] lg:text-[1.5rem] ${cryptoChanges.newOnCoinbase[crypto.id] >= 0 ? "text-green-500" : "text-red-500"}`}
                      >
                        {loading
                          ? "..."
                          : formatPercentage(
                              cryptoChanges.newOnCoinbase[crypto.id],
                            )}
                      </span>
                    </div>
                  </div>
